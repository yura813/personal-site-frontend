...
<template>
  ...
  >
    ...
    <div class="block md:hidden" @click="isMenuOpen = !isMenuOpen">
      ...
      <X class="text-white-soft" v-if="isMenuOpen" />
    </div>
  ...
  <div
    v-show="true"
    class="h-screen w-full fixed top-20 bottom-0 right-0 bg-white-soft md:hidden py-10 transition-all duration-300 ease-out transform"
    :class="
      isMenuOpen
        ? 'opacity-100'
  ...
  >
    ...
    >
      <p v-for="item in navItems" :key="item.name" @click="goTo(item.path)">
        {{ item.name }}
      </p>
    </div>
  </div>
